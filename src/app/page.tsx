                        ...
                        >
                            ...
                            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
                                {config.certificates.map((cert, index) => (
                                    <div key={index} className='group relative rounded-3xl overflow-hidden bg-slate-900 border border-white/10 hover:border-cyan-500/50 transition-all shadow-2xl'>
                                        {/* Cert Image / Icon */}
                                        <div className='aspect-[4/3] bg-gradient-to-br from-slate-800 to-slate-950 p-6 flex items-center justify-center relative overflow-hidden'>
                                            ...
                                            <img
                                                src={cert.image || '/ai_neural_processor_core_1773297711972.png'}
                                                alt={cert.name}
                                                className='max-h-full max-w-full object-contain drop-shadow-[0_0_20px_rgba(34,211,238,0.2)] transition-transform duration-500 group-hover:scale-105'
                                            />
                                        </div>

                                        {/* Content */}
                                        <div className='p-8'>
                                            <div className='text-[10px] font-bold text-cyan-400 uppercase tracking-widest mb-1'>{cert.issuer}</div>
                                            <h3 className='text-lg font-serif font-black text-white mb-3 leading-tight'>{cert.name}</h3>
                                            <div className='text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4'>{cert.date}</div>
