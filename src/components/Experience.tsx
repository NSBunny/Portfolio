                            ...
                            <div className='flex items-center justify-between gap-4 mb-4'>
                                <div className='flex items-center gap-4'>
                                    <div className='h-12 w-12 rounded-xl bg-white/10 flex items-center justify-center p-2'>
                                        ...
                                        <div className='text-[8px] font-black text-slate-500 uppercase'>{exp.company.substring(0, 3)}</div>
                                    </div>
                                    <div>
                                        <h3 className='text-white font-bold leading-tight'>{exp.role}</h3>
                                        <div className='text-cyan-400 text-xs font-medium tracking-widest uppercase mt-0.5'>{exp.company}</div>
                                    </div>
                                </div>
                                <div className='text-[10px] font-bold text-slate-500 whitespace-nowrap hidden sm:block'>{exp.duration}</div>
                            </div>
                            <ul className='space-y-3'>
                                {exp.description.map((item, i) => (
                                    <li key={i} className='text-sm text-slate-400 font-light flex items-start gap-2'>
