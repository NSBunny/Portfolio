                ...
                {projects.map((project, index) => (
                    <div key={index} className='group relative rounded-3xl overflow-hidden bg-slate-900 border border-white/10 hover:border-cyan-500/50 transition-all shadow-2xl'>
                        ...
                        <div className='aspect-video overflow-hidden relative'>
                            <img
                                src={
                                    project.name.includes('News') ? '/quick_news_project_mockup_1773303206792.png' :
                                        project.name.includes('Recognition') ? '/face_recognition_project_mockup_1773303256023.png' :
                        ...
                        <div className='p-8 relative'>
                            <div className='flex items-center justify-between mb-4'>
                                <h3 className='text-xl font-serif font-black text-white'>{project.name}</h3>
                            ...
                            <div className='flex flex-wrap gap-2'>
                                {project.technologies.map(tech => (
                                    <span key={tech} className='px-3 py-1 rounded-full bg-white/5 border border-white/5 text-[9px] font-bold uppercase tracking-widest text-cyan-400'>
                                        {tech}
                                    </span>
                                ))}
                            </div>
